...
function(unit = "month", date_format = "%B %Y", interactive = TRUE){
    ...
    data_tbl <- bike_orderlines_tbl %>%
        ...
        # Rounding the date down for monthly
    ...
    g1 <- data_tbl %>%
        ...
        # Geoms
        # to use ggplotly with our column for label we must set aes(text = label_text) here
...
function(category_1 = "All", category_2 = "All",
    ...
    g2 <- data_tbl %>%
        ...
        theme(legend.position = "none",
              # **strip.text.x = element_text() changes hight of facet titles
