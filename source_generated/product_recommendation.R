...
get_bike_features <- function() {
    ...
    bike_features_tbl <- bike_orderlines_tbl %>%
        ...
        # separate_bike_model is our prior built function which takes the model dolumn and extracted
        # useful features
...
plot_bike_features <- function(interactive = TRUE) {
    ...
    # VISUALIZATION
    g <- bike_features_tbl %>%
        # fct_reorder(category_2, price) reorders by median price by default. 
        # Can be changed by using .fun argument
        mutate(category_2 = fct_reorder(category_2, price)) %>%
        mutate(label_text = str_glue("Model: {model}
                                     Price: {scales::dollar(price)}")) %>%
        ggplot(aes(category_2, price)) +
        geom_violin() +
        # Prefer jitter to avoid overlapping points
