...
plot_customer_heatmap <- function(interactive = TRUE) {
    ...
    pct_sales_by_customer_tbl <- bike_orderlines_tbl %>%
        ...
        ungroup() %>%
        # Factoring alphabetically here, so using as.factor instead of as_factor
    ...
    g <- pct_sales_by_customer_tbl %>%
        ...
        geom_text(aes(label = scales::percent(pct, accuracy=0.1), text = label_text),
                  size = 3) +
        # Note scales = "free_x" helps to remove blank columns created in facetting
...
get_customer_segments <- function(k = 4, seed = 123) {
    ...
    customer_trends_tbl <- bike_orderlines_tbl %>%
        ...
        # group_by_at helps to speed up grouping coding of parameter naming
    ...
    # 3.0 UMAP
    # Note setting seed for umap is different than normal, we need to do this in its configuration
    ...
    umap_obj <- customer_product_tbl %>%
        select(-bikeshop_name) %>%
        as.matrix() %>%
        # here is where we input our updated umap configuration from above
...
plot_customer_behavior_by_cluster <- function(top_n_products = 10, 
    ...
    top_n_tbl <- bike_orderlines_tbl %>%
        ...
        ungroup() %>%
        group_by(bikeshop_name) %>%
        # **Note by default arrange ignores groups.  Need to use ".by_group" argument to do by group
        arrange(desc(total_qty), .by_group = TRUE) %>%
        # Getting top 10 products after arranging
...
dump(function_names, file = "../scripts/plot_customer_segmentation.R")
